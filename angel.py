...
def return_source(
    ...
    if source_indexs is not None and \
        ...
        artist_filter = \
            set(itertools.chain(*
                map(track_artist, tracks_filter)))
        if tracks_weight:
            artist_weight = collections.Counter()
            for track, weight in \
                tracks_weight.items():
                _artist = track_artist(track)
                for artist in _artist:
                    if artist in artist_filter:
                        _ = artist_weight[artist]
                        if _ == 0 or _ > weight:
                            artist_weight[artist] \
                                = weight
        prefix = \
            set(map(
                lambda _: os.path.split(_)[0],
                source_prefix))
        ...
        for artist in artist_filter:
            ...
            if not tracks_weight:
                ...
                weight = artist_weight[artist]
                for _index in indexs:
                    source[_index] += weight
        source = \
            collections.Counter({
                (file :=
                    os.path.join(
                    ...
                    fscore(
                        ...
                        tracks_recall or 1.5)
                for id, counts in
                    source.most_common()
                    if tracks_common is None or
                        counts >= tracks_common})
        if source_number:
            source = \
                collections.Counter(
                    dict(source.most_common(
                        source_number)))
        if source_search and not source:
            source_search = False
    if source_search and \
        ...
        cmdarg_tracks = \
            locate_pickle([],
                ...
                return_cmdarg=True)
        source_bundle = \
            int(0.7 * len(source or source_prefix)
                * (os.sysconf('SC_ARG_MAX') -
                    len(' '.join(cmdarg_tracks)))
                / len(' '.join(cmdarg_source)))
        _locate_pickle = \
            functools.partial(
                locate_pickle,
        ...
        source = \
            collections.Counter({
                _[0].removesuffix(
                    '.pickle') + '.json':
                        round(_[1], 3)
                for _ in _source.items()
                    if tracks_common is None or
                        _[1] >= tracks_common})
        if tracks_fscore and source:
            _cmdarg_source = \
                locate_pickle(
...
def obtain_indexs(
    ...
    if isinstance(source_prefix, str):
        if '*' in source_prefix:
            for _ in glob.glob(source_prefix):
                obtain_indexs(_, tracks_artist)
            return
    else:
        for _ in source_prefix:
            obtain_indexs(_, tracks_artist)
        return
    print(source_prefix)
    if tracks_artist:
        tracks_artist = \
            set(itertools.chain(*
                map(track_artist, tracks_artist)))
    artist_source = {}
...
def locate_pickle(
    ...
    args = (
        'rg',
        *(('--threads', str(thread_number))
            if thread_number else ()),
        *(('--count-matches',)
        ...
        for line in proc.stdout.readlines():
            ...
            if not tracks_filter:
                ...
                if string[1:] in tracks_filter:
                    string = string[1:]
                weight = \
                    1 if not tracks_weight else \
                    tracks_weight.get(string, 0)
...
def import_tracks(
    ...
    if callable(filter):
        _filter = lambda _: filter(_[1])
    elif filter:
        _filter = lambda _: _[1] in filter
    else: _filter = None
    return dict(
        functools.reduce(
...
def import_source_pickle(
    ...
    for track in (
        ...
        if (filter(track)
            ...
            index = tracks[track]
            if number:
                if min(index) >= number: continue
            if not window: idxs.update(index)
            else:
                if window == math.inf:
...
def track_id(
    ...
    **kwargs
    ...
    artist = adjust_artist(adjust_string(artist))
    title = adjust_title(adjust_string(title))
        ...
        if ((
            any(
                ...
                map(title.__contains__,
                    tracks_themes)
            ))): return f'{track} ({subtitle})'
    return track

def track_artist(track):
    artist, *_ = track.split(' - ', 1)
    for lexema, titles, phrase, spaces \
        ...
        for _spaces, _lexema in \
            ...
            if phrase and ' ' not in artist1[-1]:
                artist1[-1] += \
                    _lexema + artist2[0]
                del artist2[0]
...
def adjust_title(title):
    ...
    lindex = title.rfind('(')
    rindex = title.rfind(')')
    if dindex > lindex and dindex > rindex: 
        return adjust_dashes(title)
    else:
        return adjust_parens(title)

def adjust_dashes(title):
    _title, *suffix = title.rsplit(' - ', 1)
    if suffix:
        _title = adjust_title(_title)
        if adjust_title(f'({suffix[0]})'):
            _title += f' - {suffix[0]}'
    return _title.strip()

def adjust_parens(title):
    _depth, lindex, rindex = 0, None, len(title)
    for _index in range(len(title)-1, -1, -1):
        match title[_index]:
            case ')':
                _depth += 1
                if _depth == 1: rindex = _index
            case '(':
                _depth -= 1
                lindex = _index
                if _depth == 0: break
    if lindex is None: _title = title
    else:
        _title = title[:lindex].strip()
        parens = title[lindex+1:rindex]
        ending = title[rindex+1:]
    if _title:
        ...
        for lexema, titles, phrase, spaces \
            ...
            if titles:
                _title = \
                    _title.split(f' {lexema} ')[0]
        _title = remove_suffix(_title, ('?', '!'))
    if lindex is not None:
        lexema = parens.split(' ')
        for _lexema in lexema:
            if _lexema in tracks_tweaks:
                lexema = []
                break
        for _lexema in lexema:
            if _lexema in tracks_medias and \
                any(map(
                    _title.__contains__,
                    tracks_themes)): continue
            if _lexema in tracks_traits:
                parens = ''
                break
        if parens:
            if not parens.isdigit():
                _title += f' ({parens})'
                if ending and \
                    adjust_title(f'({ending})'):
                    _title += ending
    return _title.strip()
...
def expand_artist(tracks, collab=None):
    ...
    for track in list(tracks):
        ...
        if ': ' in title:
            author, naming = title.split(': ', 1)
            for _artist in artist:
                if author in _artist:
                    tracks.add(
                        track_id(
                            artist=_artist,
                            title=naming))
