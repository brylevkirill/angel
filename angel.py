...
def return_source(
    ...
    if source_search and \
        ...
        source = \
            collections.Counter({
                ...
                if tracks_common is None
                    or _[1] >= tracks_common
                })
        if tracks_fscore:
            _source = \
                locate_pickle(
    ...
    def worker(_):
        ...
        tracks = import_source(_source, *_params)
        if tracks:
            if not reduce:
                return _source, tracks
            else:
                return reduce([(_source, tracks)])
        else:
            return None
    params = \
        zip(source_queued,
...
def list2item(tracks, nearby, picked, artist):
    tracks = expand_artist(tracks, collab=True)
    tracks -= ignore_tracks()
    if artist:
        def artist(track):
            if type(track) == tuple:
                track = track[0] 
            return track_artist(track)[0]
        tracks |= set(map(artist, tracks))
    ...
    def artist_differ(track1, track2):
        ...
        return not (
            artist_cached[track1] &
                artist_cached[track2]
        )
    return collections.Counter(
        itertools.chain(
            *map(
...
def obtain(
    ...
    result = cutoff(
        list2item(
            tracks, nearby, picked, artist
            )
        )
    if droped:
        if droped < 0:
            droped = picked + droped
        remove = cutoff(
            list2item(
                tracks, nearby, droped, artist
                )
            )
        result = [
            track for track in result
                if track not in remove
        ]
    return result

def nearby(tracks, metric, picked=250):
    return {
...
def impact(_track, nearby, picked=250):
    return list(reversed(
        collections.Counter({
            track: index
            for track, counts in nearby.items()
                for index, _count in enumerate(
                    counts.most_common(picked)
                ) if _count[0] == _track
            }).most_common()
        ))
